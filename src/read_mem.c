...
#define verb_debug () { \
	...
	printf ("+- Swap total : %ld, used : %ld, free : %ld \n", state_G.fresh.swap_total, state_G.fresh.swap_used, state_G.fresh.swap_free); \
}
#else
#define verb_debug ()
#endif /* DEBUG */
...
read_meminfo (void)
{
	...
	state_G.fresh.swap_total = get_num (file_pG, "SwapTotal") * 1024;
	state_G.fresh.swap_free = get_num (file_pG, "SwapFree") * 1024;
	state_G.fresh.swap_used = state_G.fresh.swap_total - state_G.fresh.swap_free;
...
{
	int result;
	if ((file_pG = fopen (state_G.proc_mem_filename, "r")) == NULL) {
		error_handle (1, "");
		return -1;
	}
	return 0;
